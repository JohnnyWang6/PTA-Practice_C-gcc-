...
int main(){
    ...
    */
    ...
    scanf("%d",&N);
    int label[N][10];//定义label二维数组存储每篇博文的点赞特性标签。
    int count[1001]={0};//定义count一维数组存储特性标签出现的次数，初值为0。
    for(i=0; i<N; i++){ //接着循环输入每篇博文的点赞特性
        scanf("%d",&K);
        for(j=0; j<K; j++){
            scanf("%d",&label[i][j]);
            count[label[i][j]]++;
    ...
    max = count[0];
    maxIndex = 0;
    for(i=1; i<1001; i++){ //开始循环比较出count数组中最大的值，即点赞特性出现最多的特性。
