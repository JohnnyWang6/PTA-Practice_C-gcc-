...
int difCount(int y, int flag){ //定义方法算当前年份的不同数字有多少个，传递的形参需要传一个flag判断当前年份是不是小于1000
    ...
    while(temp != 0){
        remainder = temp % 10;
        temp /= 10;
        if(flag==1){ //如果flag=1说明y<1000,前面补了0，所以y中的0就不再纳入计算了
            if(remainder==0){
                continue;
            }
        }
...
int main(){
    ...
    */
    ...
    scanf("%d %d",&y, &n);
    temp = y;
    if(y>=1000){ //如果当前年份大于1000，则flag还是为0
        dif = difCount(y,flag); //求出一开始的dif值
        if(n==dif){ //如果n等于dif，直接输出0岁就到了目标年份
            printf("0 %04d\n",y);
        }else{ //如果n不等于dif,就循环++年份，并不断调用difCount求出dif的值，直到n和dif相等
            while(dif!=n){
                y++;
                age++;
                dif = difCount(y,flag);
    ...
    }else{ //如果一开始的年份小于1000，则flag设为1，代表这类年份本身的dif就要加1，因为前面会补0
        flag = 1;
        dif = 1 + difCount(y,flag); //如果年份小于1000，用difCount函数求dif的值时要加1，表示前面补了一个0
        if(n == dif){ //如果一开始的n就等于dif，直接输出0岁就到了目标年份
            printf("0 %04d\n",y);
        }else{
            while(dif!=n){
                y++;
